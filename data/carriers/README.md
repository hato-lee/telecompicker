...
- ⭐ **데이터 통로 (2026-09-01 뚫음)**: `POST /smc/plan/planCompareResult.do`
  ...
  - ⚠️ **연속 요청 6번쯤에서 대기 페이지(HTML)로 막힌다.** 세션 쿠키(목록 페이지 GET) 필수.
    5초 간격으로도 막혔고, **막히면 3분으로는 안 풀린다** (2026-09-01 실측 — 60초×4 재시도도 실패).
    → 재수집은 `scripts/fetch-smartchoice-list-slow.mjs` (15분 식힘 + 요청 간 20초 + 막히면 5분).
    **그래도 안 되면 playwright(브라우저)로 간다** — 브라우저 세션은 쿠키·JS 문제없이
    페이지 안 fetch로 잘 뚫린다(2026-09-01에 1~6페이지 600개 성공). 간격만 넉넉히 둘 것.
