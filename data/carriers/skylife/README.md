...
- **일 단위**: `properties.data.description`에 「매일」이 들어간 21개 중 **20개는 「모두 충분 11GB+/데이터 충분 …GB+」류로 라운드 1에 이미 담겨 있었다**(예: 「소진시 매일 2GB+3Mbps 속도로 무제한」 — 월 기본량이 있어 dataGB만으로도 뜻이 통했다. dailyDataGB 칸이 없어 「매일 2GB」 부분만 못 담았다). ⚠️ 기존 항목이라 이번 라운드에서 고치지 않았다. **유일하게 완전히 빠져 있던 것은 `pl221o962`(모두 충분 일5GB+(왓챠))** — 월 기본량 없이 매일 5GB만 주는 진짜 「일 단위만」 구조라 dataGB=0으로 처음 담았다.
- **연령 전용**: `tags.json`의 `주니어`(4개, 「만 19세 미만 가입 가능」→ageMax=18) · `시니어`(6개, 「만 65세 이상 가입 가능」→ageMin=65)를 전부 담았다. `복지 전용`(1개) · `스마트기기`(3개)는 DECIDED 기준으로 계속 뺐다.
