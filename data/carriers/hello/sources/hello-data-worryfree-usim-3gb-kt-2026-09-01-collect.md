# 데이터 걱정없는 USIM 3GB — 원문에서 본 값 (2026-09-01 확인)
...
| 칸 | 원문 |
|---|---|
| 요금제명 | 데이터 걱정없는 USIM 3GB |
| 머리줄(데이터) | 3GB +1Mbps 무제한 |
| 망 | KT 망 |
| 세대 | LTE |
| 통화 | 기본제공 |
| 문자 | 기본제공 |
| 분류 딱지 | (없음) |
| 가격 옆 딱지 | 평생요금 |
| 월 요금 | 25,950원 (부가세 포함 — 유의사항 「월 기본료 및 국내 통화료는 부가세 포함금액입니다」) |
...
```json
{
 "idx": "235",
 "pidx": "66",
 "paymentcode": "PDB0000261",
 "salesName": "데이터 걱정없는 USIM 3GB",
 "salesBadge": "",
 "telecom": "KT",
 "title": "LTE USIM 요금제",
 "usimType": "LTE",
 ...
 "directPromotionDirectmallPrice": "25950",
...
```json
{
 "id": "hello-data-worryfree-usim-3gb-kt",
 "carrier": "헬로모바일",
 "carrierType": "mvno",
 "network": "KT",
 "name": "데이터 걱정없는 USIM 3GB",
 "generation": "LTE",
 "monthlyFee": 25950,
 "promo": null,
 "dataGB": 3,
 ...
 "memo": "원문 가격 옆 표시 「평생요금」 — 이 값이 계속 내는 월정액이다(기간 한정 할인 아님). 부가세: 원문 유의사항 「월 기본료 및 국내 통화료는 부가세 포함금액입니다」. 영상·부가통화 300분(스키마에 그릇 없음). 원문 요금제코드 PDB0000261."
}
```

### 왜 이 값인가

- `monthlyFee` 25,950원 — 원문 가격 옆 딱지가 「평생요금」이다. 기간 한정 할인이 아니라 계속 내는 값이므로 `promo`는 `null`.
- `dataGB` 3 / `throttleMbps` 1
