# LTE USIM 표준 — 원문에서 본 값 (2026-09-01 확인)
...
| 분류 딱지 | (없음) |
| 가격 옆 딱지 | 평생요금 |
| 월 요금 | 9,790원 (부가세 포함 — 유의사항 「월 기본료 및 국내 통화료는 부가세 포함금액입니다」) |
...
```json
{
 "idx": "106",
 "pidx": "202",
 "paymentcode": "PDB0000074",
 "salesName": "LTE USIM 표준",
 "salesBadge": "",
 "telecom": "KT",
 "title": "LTE USIM 요금제",
 "usimType": "LTE",
 ...
 "directPromotionDirectmallPrice": "9790",
...
```json
{
 ...
 "memo": "원문 가격 옆 표시 「평생요금」 — 이 값이 계속 내는 월정액이다(기간 한정 할인 아님). 부가세: 원문 유의사항 「월 기본료 및 국내 통화료는 부가세 포함금액입니다」. 원문 요금제코드 PDB0000074."
}
```

### 왜 이 값인가

- `monthlyFee` 9,790원 — 원문 가격 옆 딱지가 「평생요금」이다. 기간 한정 할인이 아니라 계속 내는 값이므로 `promo`는 `null`.
- `dataGB` 0 / `throttleMbps` null (소진 후 제공 없음)
- `voiceMinutes` 0 / `smsIncluded` false
