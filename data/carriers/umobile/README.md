...
- **일 단위(순수, 월 기본량 없음)**: `pln-spc`가 정확히 `일5GB+5Mbps`인 3개(USIM `LTE 스페셜 프로`·`GS25 (150GB+/통화기본)`, PHONE `[휴대폰] LTE 스페셜 프로`)를 dataGB=0, dailyDataGB=5로 새로 담았다.
  ⚠️ `pln-spc`가 `11GB+일2GB+3Mbps`인 항목은 **18개**(대부분 `71GB+/통화기본` 이름의 "K-패스"·"플러스알파" 계열)가 있는데 **17개는 라운드 1에 이미 dataGB=11로 담겨 있었다**(dailyDataGB 칸이 없어 매일 2GB 부분만 못 담았다 — seven·skylife와 같은 패턴, ⚠️ 기존 항목이라 이번엔 안 고쳤다). 나머지 1개(`이십세 스페셜`)는 원래 연령 전용으로 통째로 빠져 있어 이번에 새로 담았다.
- **연령 전용**: 원문 문구 그대로 인용 가능한 것만 담았다 — 틴에이저 3개(「만 18세 이하」→ageMax=18), 시니어 3개(카드 배지 이미지 `alt="bg_만65세"` + 본문 「65세 이상 가입 가능」→ageMin=65, 본문 자체엔 「만」자가 빠져 있어 배지를 근거로 썼다), [휴대폰]청소년 1개(「만 4세 이상, 만 18세 이하」→ageMin=4·ageMax=18).
  ⚠️ **「이십세」 이름의 6개 중 나이 문구가 실제로 있는 건 2개뿐이다** — `이십세 (15GB+/300분)`·`이십세 스페셜`(둘 다 「본 요금제는 만 19세 이상부터 만 29세 이하만 가입이 가능합니다」)만 담았다. 나머지 4개(`이십세 (10GB+/500분)`·`(1GB+/통화기본)`·`(4.5GB+/통화기본)`·`(7GB+/통화기본)`)는 상세 원문 전체를 뒤져도 나이 제한 문구가 전혀 없다 — 「YOUNG」 판례와 같은 이유로 계속 안 담았다.
  **갤럭시틴즈 3개**는 [휴대폰]청소년과 똑같은 「만 4세 이상, 만 18세 이하」 문구를 갖고 있지만, 이름 자체가 특정 단말(갤럭시) 결합을 시사해 총괄 지시로 계속 뺐다 — 원문에 별도 "갤럭시 전용" 조건 텍스트가 명시적으로 있는 건 아니라서, 다음 라운드가 재검토하려면 이 판단 근거(이름만)를 알아야 한다.
