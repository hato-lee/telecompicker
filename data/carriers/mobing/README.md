...
| 무엇 | 개수 | 고른 방법 |
|---|---|---|
| 일 단위 데이터 | ~~22~~ **0** (2026-09-01 라운드 2에서 22건 전부 담음, `dailyDataGB` 신설) | `dayDataFlag='Y'` 또는 `dailyQos` 있음 |
| 가입 대상 제한(청소년·시니어·복지·주니어) | 14 → 세분화: **복지 6건은 계속 제외**(모빙 복지 250분3GB·복지 우리끼리·모빙 복지 100분1GB·쏙쏙 복지 100분1GB·쏙쏙 복지 250분3GB·복지 우리끼리 LTE), **나이 전용 8건 중 7건은 라운드 2에서 담음**(`ageMin`/`ageMax` 신설, 상세 `contents`의 「만 N세 이하/이상만 가입이 가능합니다」 문구 인용), **1건(주니어알뜰 표준, PC0LB00032)은 여전히 못 담음** — 상세 API `networkID`가 `3G`인데 스키마 `generation`에 3G 그릇이 없다(⚠️ 결정 재검토 제안: 3G 값 신설 여부) | 이름 검사 |

226건 중 **190건**을 담았다. **라운드 2(2026-09-01)에서 일 단위 22건 + 나이 전용 7건 = 29건 추가, 총 219건.**
