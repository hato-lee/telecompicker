...
## 라운드 2 갱신 (2026-09-01) — 일 단위 데이터 31개 + 시니어 1개를 담았다

게이트 2 결정(`dailyDataGB` 그릇 신설 + 나이 질문 추가)으로 아래 32개를 `tplus.json`에 **추가**했다
(기존 195개는 그대로, 총 227개).

- 일 단위 31개: 카드 데이터 표시 「NGB 일 MGB 최대 X Mbps」/「일 MGB 최대 X Mbps」→ `dataGB N(또는 0), dailyDataGB M, throttleMbps X`.
  값(`monthlyFee`/`promo`)은 위 「`.desc16px700b50` 라벨」 표를 그대로 적용 — 「N개월 후 …」 30개, 「평생 …」 1개.
- 시니어 1개(완전마음껏 시니어2G＋, KT): 배지 「시니어 요금제(만65세 이상)」를 그대로 인용해 `ageMin=65`.
  값 「6개월 후 평생 8,800원, 만65세이상 가능」→ `monthlyFee=8800, promo={months:6, feeDuring:5500}`.
- 근거·전체 목록: `sources/daily-batch-2026-09-01-round2-daily-and-senior.md`.

## 안 담은 것 (2026-09-01 · 235개 중 8개, 라운드 2 갱신)
