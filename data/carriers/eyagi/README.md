...
| 태블릿·디바이스 | 4 | 태그 25 |
| 복지 | 2 | 태그 41 |
| 청소년 | 2 | 이름에 「청소년」 — 상세 페이지 문구 「만 19세이하만 가입 가능」 확인. ⚠️ **2건 다 못 담았다**: `call_str`/`sms_str`가 「52000팡」·「20000팡」(자체 포인트 단위)이고, 상세 페이지 어디에도 1팡=몇 분/몇 원 환산 규칙이 없다. `voiceMinutes`(분)·`smsIncluded`로 지어낼 수 없어 **결정 재검토 제안**으로 남긴다 |
| 일 단위 데이터 | ~~33~~ **0** (2026-09-01 라운드 2에서 33건 전부 담음, `dailyDataGB` 신설) | `add_data`에 「매일」 |

329건 중 **264건**을 담았다. **라운드 2(2026-09-01)에서 일 단위 데이터 33건을 추가로 담아 297건.**
청소년 2건은 여전히 「팡」 단위 문제로 못 담았다 — 담당 갈래 보고서에 ⚠️로 남김.
